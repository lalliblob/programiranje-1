...
let rec multiply sez =
  match sez with
  | [] -> 1  (* Za prazni seznam vrne 1, ker je to nevtralni element za množenje *)
  | x :: xs -> x * multiply xs  (* Množimo prvi element z rezultatom rekurzije *)
...
let rec sum_int_pairs (pair_list : (int * int) list) : int list = 
  match pair_list with
  | [] -> []  (* Če je seznam prazen, vrne prazen seznam *)
  | (x, y) :: tail -> (x + y) :: sum_int_pairs tail  (* seštej prvi par in rekurzivno pokliče na preostanek seznama *)
...
let rec get k list =
  match list with
  | [] -> None  (* Če je seznam prazen, vrnemo napako *)
  | x :: _ when k <= 0 -> Some x  (* Če je k negativen ali nič, vrnemo prvi element *)
  | _ :: xs -> get (k - 1) xs  (* Sicer zmanjšamo k in iščemo naprej po seznamu *)
